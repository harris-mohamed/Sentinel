...
def telegram_parse(data):
    ...
    for message in data:
        ...
        if (initial == 0):
            ...
            for nibble in message:
                curr += bytearray.fromhex(nibble).decode()
            device_info['Time since start-up'] = type_conv(curr, 'u32') 
            initial = initial + 1
        elif (initial == 8):
            curr = ''
            for nibble in message:
                curr += bytearray.fromhex(nibble).decode()
            device_info['Time of transmission'] = type_conv(curr, 'u32') 
            initial = initial + 1
        elif (initial == 9):
            curr = ''
            for nibble in message:
...
# AWS Code
def createDeviceItem():
    response = table.put_item(
        Item={
            'Time-stamp': str(datetime.now()),
            'Version-Number': device_info['Version Number'],
            'Device-Number': device_info['Device Number'],
            'Serial-Number': device_info['Serial Number'],
            'Device-Status': device_info['Device Status'],
            'Telegram-Counter': device_info['Telegram Counter'],
            'Scan-Counter': device_info['Scan Counter'],
            'Time-since-start-up': device_info['Time since start-up'],
            'Time-of-transmission': device_info['Time of transmission'],
            'Status-of-digital-inputs': device_info['Status of digital inputs'],
            'Status-of-digital-outputs': device_info['Status of digital outputs'],
            'Layer-Angle': device_info['Layer Angle'],
            'Scan-Frequency': device_info['Scan Frequency'],
            'Measurement-Frequency': device_info['Measurement Frequency'],
            'Amount-of-Encoder': device_info['Amount of Encoder'],
            '16-bit-Channels': device_info['16-bit Channels']
        }
    )

def createTelegramItem():
    response = table.put_item(
        Item={
            'Time-stamp': str(datetime.now()),
            'Message-count': telegram_structure['Message Count'],
            'Scale-factor': telegram_structure['Scale Factor'],
            'Scale-factor-offset': telegram_structure['Scale Factor Offset'],
            'Start-Angle': telegram_structure['Start Angle'],
            'Angular-Increment': telegram_structure['Angular Increment'],
            'Quantity': telegram_structure['Quantity'],
            'Measurement': telegram_structure['Measurement']
        }
    )
...
def parser():
    ...
    for message in clean_objects:
        ...
        remaining = message[2:]
        telegram_parse(remaining)
        parsed_messages.append(telegram_structure)

        with open("output.txt", "w") as f:
            for i in range(len(parsed_messages)):
                f.write(str(parsed_messages[i]))
